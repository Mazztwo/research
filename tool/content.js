 ...
 *
 *  This file has been modified with additional functions for the project
 *
 */
...
/*
   Once this script has been injected into the page, loading has finished,
   and the port connection triggers the activation of the background
	 functions
*/

console.error("_BEGIN_DOM_");
console.error(JSON.stringify(domJSON.toJSON(document.body)));
console.error("_END_DOM_");

var port = chrome.runtime.connect({name: "connection"});

/*
   Adds a listener to the onMessage event to get data from the background
   script and outputs it to the console

   Note that Selenium is not picking up console.log for some reason, but is
   picking up console.error, so this script utilizes console.error()
*/
port.onMessage.addListener(function(background) {
